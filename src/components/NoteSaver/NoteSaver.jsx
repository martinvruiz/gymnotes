  ...
  const handleChange = (e) => {
    ...
    if (name === "sets") {
      ...
      setFormData((prev) => ({
        ...prev,
        [name]: setsData,
        rreps: Array.from({ length: setsData }, () => ({ reps: "", kg: "" })),
      }));
    } else {
      setFormData((prev) => ({
        ...prev,
  ...
  return (
    <>
      ...
      >
        ...
        {Array.isArray(formData.reps) &&
          formData.reps.map((set, index) => (
            <div key={index} className="w-full flex items-center my-1">
              <label htmlFor={`rep-${index}`}>Set {index + 1}:</label>
              <input
                type="number"
                id={`rep-${index}`}
                value={set.reps}
                onChange={(e) =>
                  handleRepsChange(index, "reps", e.target.value)
                }
